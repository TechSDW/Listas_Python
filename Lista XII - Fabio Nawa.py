...
def inicio_final(a, b):
  if len(a) % 2 == 0:
    ai = str((a[:int(len(a)/2)]))
    af = str((a[int(len(a)/2):]))
  else:
    ai = str((a[:int((len(a)/2)+0.5)]))
    af = str((a[int(len(a)/2+0.5):]))

  if len(b) % 2 == 0:
    bi = str((b[:int((len(a)/2)-1)]))
    bf = str((b[int((len(a)/2)-1):]))
  else:
    bi = str((b[:int(len(a)/2)]))
    bf = str((b[int(len(a)/2):]))

  return str(ai + bi + af + bf)
